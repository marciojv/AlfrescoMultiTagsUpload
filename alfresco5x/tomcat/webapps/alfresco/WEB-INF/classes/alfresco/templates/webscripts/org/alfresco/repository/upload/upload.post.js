   ...
   try
   {
      ...
      for each (field in formdata.fields)
      {
         switch (String(field.name).toLowerCase())
         {
			...
			case "prop_cm_taggable":
               m_tags = field.value;
               break;
			 
			case "prop_cm_categories":
               m_categories = field.value;
               break;
         }
      }
      ...
      else
      {
		 ...
		 addTags(newFile, m_tags);
		 addCategories(newFile, m_categories);
...
function addTags(docNode, m_tags)
{
	if(m_tags != null && m_tags != "")
	{
		var tagsArr = m_tags.split(",");
		for(var i=0; i < tagsArr.length; i++)
		{
			var tagNode = search.findNode(tagsArr[i]);
			if(tagNode != null)
			{
				docNode.addTag(tagNode.properties.name);
			}
		}						
	}
}

function addCategories(docNode, m_categories)
{
	if(m_categories != null && m_categories != "")
	{
		var categoriesArr = m_categories.split(",");
		if(!docNode.hasAspect("cm:generalclassifiable"))
		{
			docNode.addAspect("cm:generalclassifiable");
		}
		docNode.properties["cm:categories"] = categoriesArr;
		docNode.save();
	}
}
